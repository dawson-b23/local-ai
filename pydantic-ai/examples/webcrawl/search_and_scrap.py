...
from crawl4ai import LLMConfig, AdaptiveCrawler
from flare_bypasser import flare_bypasser
from crawl4ai.content_filter_strategy import LLMContentFilter


async def aggregate_links(query):
    results = DDGS().text(query, max_results=3)

    webpages_list = []
    for item in results:
        weblink = item['href'] 
        webpages_list.append(weblink)

    print(f" List of Pages to scrap: {webpages_list}")
    
    return webpages_list

async def crawl(webpages_list, query):
    ...
    if summaries:
        ...
        # Output the final aggregated summary
        print("\nFinal Aggregated Summary:\n")
        print(final_summary)
        
        # Save to file
        with open("aggregated_summary.md", "w", encoding="utf-8") as f:
            f.write("\n".join(final_summary))
...
async def main():
    ...
    target = await aggregate_links(query)
    await crawl(target, query)
