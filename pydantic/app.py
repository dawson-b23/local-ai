...
async def main_app():
    ...
    with st.form(key="chat_form", clear_on_submit=True):
        ...
        if submit_button and user_input:
            langfuse.update_current_trace(metadata={"query": user_input})
            st.session_state.messages.append({"role": "user", "content": user_input})
            async with httpx.AsyncClient() as client:
                try:
                    response = await client.post(
                        FASTAPI_URL,
                        json={"chatInput": user_input, "sessionId": ""},
                        timeout=30.0
                    )
                    response.raise_for_status()
                    result = response.json().get("response", "Error: No response from model")
                    langfuse.update_current_trace(metadata={"status": "success", "response": result})
                    st.session_state.messages.append({"role": "assistant", "content": result})
                except Exception as e:
                    logger.error(f"Error in response: {str(e)}", exc_info=True)
