...
async def query_press20_data(ctx: RunContext[Deps], sql_query: str) -> str:
    if not sql_query.strip():
        logger.error("Empty SQL query received in query_press20_data")
        return "Error: Empty SQL query provided."
    try:
        logger.debug(f"Executing SQL query: {sql_query}")
        response = await asyncio.to_thread(
            supabase.rpc("query_press20_data", {"query_text": sql_query}).execute
        )
        ...
        for row in response.data:
            ...
            formatted_row = "\n".join([f"- **{key}**: {value}" for key, value in json_data.items() if value is not None])
